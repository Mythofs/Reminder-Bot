...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (!commandPath)
            return await interaction.editReply(`Could not find the file for command \`${commandName}\`!`);

        delete require.cache[require.resolve(commandPath)];
        try {
            const newCommand = require(commandPath);
            interaction.client.commands.set(newCommand.data.name, newCommand);
            return await interaction.editReply(`Command \`${commandName}\` was reloaded!`);
        }
        catch (error) {
            console.error(error);
            return await interaction.editReply(`There was an error while reloading \`${commandName}\`:\n\`${error.message}\``);
        }
    }
}